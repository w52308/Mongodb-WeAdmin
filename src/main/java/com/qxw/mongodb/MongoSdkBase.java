...
public class MongoSdkBase {
    ...
    public MongoCollection<Document> getColl(String tableName) {
        MongoDatabase db = mongoClient.getDatabase(ommSpcDB);
     ...
     * @return
     */
    public JSONObject getGroupPage(MongoCollection table, Bson filter, Bson group, Bson sorts, int pageNum,
        int pageSize) {
        List<JSONObject> list = new ArrayList<>();
        int startRow = pageNum > 0 ? (pageNum - 1) * pageSize : 0;
        MongoCursor<Document> iterator = table.aggregate(
            Arrays.asList(match(filter), group, sort(sorts), skip(startRow), limit(pageSize))).iterator();
        while (iterator.hasNext()) {
    ...
    public static final String diyObjectIdToJson(Object object) {
        return JSON.toJSONString(object, objectIdSerializer, SerializerFeature.WriteDateUseDateFormat,
